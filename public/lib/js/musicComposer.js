const __G_AUDIO_CTX = new window.AudioContext();

function GTone(tone, duration = 200, volume = 100) {
  const frequency = GTone.prototype.tone2freq(tone);

  this.play = () => {
    const oscillator = __G_AUDIO_CTX.createOscillator();
    const gainNode = __G_AUDIO_CTX.createGain();

    oscillator.connect(gainNode);
    gainNode.connect(__G_AUDIO_CTX.destination);
